...
ukvminit(struct proc *p)
{
  ...
  ukvmmap(p->kpagetable, CLINT, CLINT, 0x10000, PTE_R | PTE_W);
  ukvmmap(p->kpagetable, PLIC, PLIC, 0x400000, PTE_R | PTE_W);
  ukvmmap(p->kpagetable, KERNBASE, KERNBASE, (uint64)etext-KERNBASE, PTE_R | PTE_X);
  ukvmmap(p->kpagetable, (uint64)etext, (uint64)etext, PHYSTOP-(uint64)etext, PTE_R | PTE_W);
  ukvmmap(p->kpagetable, TRAMPOLINE, (uint64)trampoline, PGSIZE, PTE_R | PTE_X);
}
...
int      
ukvmcopy(pagetable_t old, pagetable_t new, uint64 old_sz, uint64 new_sz)
{
  ...
  for(i = old_sz; i < new_sz; i += PGSIZE){
    if((old_pte = walk(old, i, 0)) == 0)
      panic("ukvmcopy: old pte should exist");
    if((*old_pte & PTE_V) == 0)
      panic("ukvmcopy: old page not present");
    if((new_pte = walk(new, i, 1)) == 0)
      panic("ukvmcopy: new pte should exist");
    pa = PTE2PA(*old_pte);
    flags = PTE_FLAGS(*old_pte & ~PTE_U);
